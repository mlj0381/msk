...
class store_mdl_goods_cat extends dbeav_model {
     ...
     * 删除分类.
     */
    public function remove($catid, $store_id, &$msg = '') {
        if ($this->count(array('parent_id' => $catid))) {
            $msg = '该分类下面还有子分类';
            return false;
        }
        if (app::get('b2c')->model('goods')->count(array('cat_id' => $catid, 'store_id' => $store_id))) {
