...
$.validator.messages = {
 ...
 $.extend(true, $.validator.methods, {
	...
	tocompare : function(value, element, param)
	{
		if (this.optional( element ) ) {
			return "dependency-mismatch";
		}
		var box = $(element).parent(); //.find('[compare]');
		var format = $(element).attr('format');
		var lb = $(box).find('input[tocompare="lb"]').val();
		var ub = $(box).find('input[tocompare="ub"]').val();
		if(lb == 'undefined' || ub == 'undefined') return false;
		if(format == 'number')
		{
			lb = parseInt(lb);
			ub = parseInt(ub);
		...
		{
			var lbStr = lb.replace(/-/g,"/");
				lb = new Date(lbStr);
			var ubStr = ub.replace(/-/g,"/");
				ub = new Date(ubStr);
			if(	/Invalid|NaN/.test( new Date( lb ).toString()) ||
				/Invalid|NaN/.test( new Date( ub ).toString())
			)
			{
				return false;
			}
			console.log(Date.parse(ub), Date.parse(lb));
			return Date.parse(ub) > Date.parse(lb);
		}else if(format == 'time')
		{
			var d = new Date();
			var date = $.VMC.format("{0}-{1}-{2}", d.getFullYear(), d.getMonth() + 1, d.getDate());
			console.log(date);
			lb = DateTime.parse("2010-1-9 " + lb + ":00");
		}
	}
});
	...
	$('form[data-module="validator"]').each(function(n, obj)
	{
		var initRule = function (Obj){
			...
			if ( rules.compare ) {
				param = rules.compare;
				delete rules.remote;
				rules = $.extend( rules, { compare: param });
			}
		...
		$(obj).find('input[required]').each(function(i, item){
			...
			rules[name] = $.extend(true,
				$.validator.attributeRules(item),
				$.validator.dataRules(item)
			);
			//rules[name] = $.validator.attributeRules(item);
			/*
